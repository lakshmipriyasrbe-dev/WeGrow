...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['closure_date']) && empty($action)) {
    ...
    /* else {
        ...
        if (empty($student_records)) {
            ...
            // Check if student has paid the fees fully
            $fees_amount = floatval($student_records[0]['fees_amount'] ?? 0);
            ...
            $pay_res = $bf->getQueryRecords($pay_query, [':enrollment_id' => $student_id]);
            $total_paid = !empty($pay_res[0]['total_paid']) ? floatval($pay_res[0]['total_paid']) : 0.0;
            
            if ($total_paid < $fees_amount) {
                $pending_balance = $fees_amount - $total_paid;
                $errors['student_id'] = 'Still pending payments are there (Pending: Rs. ' . number_format($pending_balance, 2) . '). Student must pay the full fees before course closure.';
            }

            // Check if student has finished 50% of course duration
            $doj = $student_records[0]['doj'] ?? '';
            $duration = floatval($student_records[0]['duration'] ?? 0);
            
            if (!empty($doj) && $duration > 0 && !empty($closure_date)) {
                $date = new DateTime($doj);
                if ($course_type === 'internship') {
                    // Duration in days
                    $days_to_add = round($duration / 2);
                    $date->modify('+' . $days_to_add . ' days');
                } else {
                    // Duration in months
                    $months_to_add = $duration / 2;
                    $whole_months = floor($months_to_add);
                    $fraction_months = $months_to_add - $whole_months;
                    if ($whole_months > 0) {
                        $date->modify('+' . $whole_months . ' months');
                    }
                    if ($fraction_months > 0) {
                        $days = round($fraction_months * 30);
                        $date->modify('+' . $days . ' days');
                    }
                }
                $min_closure_date = $date->format('Y-m-d');
                if (strtotime($closure_date) < strtotime($min_closure_date)) {
                    $errors['closure_date'] = 'Minimum 50% of duration must be finished. Allowed course closure date is ' . date('d-m-Y', strtotime($min_closure_date));
                }
            }
        }
    } 
