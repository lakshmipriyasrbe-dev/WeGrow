...
// ----------------------------------------------------
// Prefetch and aggregate metrics for all branches to eliminate N+1 queries in the loop
$tasks_by_company = [];
$raw_tasks = $bf->getQueryRecords("SELECT company_id, 
       COUNT(CASE WHEN status != 'completed' THEN 1 END) as pending_tasks,
       COUNT(CASE WHEN status = 'completed' THEN 1 END) as completed_tasks
FROM {$GLOBALS['task_table']}
WHERE deleted = 0
GROUP BY company_id");
foreach ($raw_tasks as $r) {
    $tasks_by_company[$r['company_id']] = [
        'pending' => intval($r['pending_tasks']),
        'completed' => intval($r['completed_tasks'])
    ];
}

$reports_by_company = [];
$raw_reports = $bf->getQueryRecords("SELECT company_id, COUNT(*) as total 
FROM {$GLOBALS['report_table']}
WHERE deleted = 0
GROUP BY company_id");
foreach ($raw_reports as $r) {
    $reports_by_company[$r['company_id']] = intval($r['total']);
}

$enquiries_by_company = [];
$raw_enquiries = $bf->getQueryRecords("SELECT company_id,
       COUNT(CASE WHEN DATE(created_date_time) = :today THEN 1 END) as today_count,
       COUNT(CASE WHEN DATE(created_date_time) BETWEEN :ws AND :we THEN 1 END) as week_count,
       COUNT(CASE WHEN DATE(created_date_time) BETWEEN :ms AND :me THEN 1 END) as month_count
FROM {$GLOBALS['course_enquiry_table']}
WHERE deleted = 0
GROUP BY company_id", [':today' => $today, ':ws' => $week_start, ':we' => $week_end, ':ms' => $month_start, ':me' => $month_end]);
foreach ($raw_enquiries as $r) {
    $enquiries_by_company[$r['company_id']] = [
...
$enrollments_by_company = [];
$raw_enrollments = $bf->getQueryRecords("SELECT company_id,
       COUNT(CASE WHEN DATE(doj) = :today THEN 1 END) as today_count,
       COUNT(CASE WHEN DATE(doj) BETWEEN :ws AND :we THEN 1 END) as week_count,
       COUNT(CASE WHEN DATE(doj) BETWEEN :ms AND :me THEN 1 END) as month_count
FROM {$GLOBALS['enrollment_table']}
WHERE deleted = 0
GROUP BY company_id", [':today' => $today, ':ws' => $week_start, ':we' => $week_end, ':ms' => $month_start, ':me' => $month_end]);
foreach ($raw_enrollments as $r) {
    $enrollments_by_company[$r['company_id']] = [
        'today' => intval($r['today_count']),
        'week' => intval($r['week_count']),
        'month' => intval($r['month_count'])
    ];
...
$raw_internships = $bf->getQueryRecords("SELECT company_id,
       COUNT(CASE WHEN DATE(doj) = :today THEN 1 END) as today_count,
       COUNT(CASE WHEN DATE(doj) BETWEEN :ws AND :we THEN 1 END) as week_count,
       COUNT(CASE WHEN DATE(doj) BETWEEN :ms AND :me THEN 1 END) as month_count
...
WHERE deleted = 0
GROUP BY company_id", [':today' => $today, ':ws' => $week_start, ':we' => $week_end, ':ms' => $month_start, ':me' => $month_end]);
foreach ($raw_internships as $r) {
    $internships_by_company[$r['company_id']] = [
        'today' => intval($r['today_count']),
        'week' => intval($r['week_count']),
        'month' => intval($r['month_count'])
    ];
...
$payments_by_company = [];
$raw_payments = $bf->getQueryRecords("SELECT company_id,
       COALESCE(SUM(CASE WHEN DATE(payment_date) = :today THEN total_amount END), 0) as today_sum,
       COALESCE(SUM(CASE WHEN DATE(payment_date) BETWEEN :ws AND :we THEN total_amount END), 0) as week_sum,
       COALESCE(SUM(CASE WHEN DATE(payment_date) BETWEEN :ms AND :me THEN total_amount END), 0) as month_sum
FROM {$GLOBALS['payment_table']}
WHERE deleted = 0
GROUP BY company_id", [':today' => $today, ':ws' => $week_start, ':we' => $week_end, ':ms' => $month_start, ':me' => $month_end]);
foreach ($raw_payments as $r) {
    $payments_by_company[$r['company_id']] = [
        'today' => floatval($r['today_sum']),
        'week' => floatval($r['week_sum']),
        'month' => floatval($r['month_sum'])
    ];
}

$expenses_by_company = [];
$raw_expenses = $bf->getQueryRecords("SELECT company_id,
       COALESCE(SUM(CASE WHEN DATE(expense_entry_date) = :today THEN total_amount END), 0) as today_sum,
       COALESCE(SUM(CASE WHEN DATE(expense_entry_date) BETWEEN :ws AND :we THEN total_amount END), 0) as week_sum,
       COALESCE(SUM(CASE WHEN DATE(expense_entry_date) BETWEEN :ms AND :me THEN total_amount END), 0) as month_sum
FROM {$GLOBALS['expense_entry_table']}
WHERE deleted = 0
GROUP BY company_id", [':today' => $today, ':ws' => $week_start, ':we' => $week_end, ':ms' => $month_start, ':me' => $month_end]);
foreach ($raw_expenses as $r) {
    $expenses_by_company[$r['company_id']] = [
        'today' => floatval($r['today_sum']),
        'week' => floatval($r['week_sum']),
        'month' => floatval($r['month_sum'])
    ];
...
foreach ($companies as $comp) {
    ...
    $pending_tasks = $tasks_by_company[$cid]['pending'] ?? 0;
    $completed_tasks = $tasks_by_company[$cid]['completed'] ?? 0;
    $reports_submitted = $reports_by_company[$cid] ?? 0;

    $enq_today = $enquiries_by_company[$cid]['today'] ?? 0;
    $enq_week  = $enquiries_by_company[$cid]['week'] ?? 0;
    $enq_month = $enquiries_by_company[$cid]['month'] ?? 0;

    $enroll_today = ($enrollments_by_company[$cid]['today'] ?? 0) + ($internships_by_company[$cid]['today'] ?? 0);
    $enroll_week  = ($enrollments_by_company[$cid]['week'] ?? 0) + ($internships_by_company[$cid]['week'] ?? 0);
    $enroll_month = ($enrollments_by_company[$cid]['month'] ?? 0) + ($internships_by_company[$cid]['month'] ?? 0);

    $pay_today = $payments_by_company[$cid]['today'] ?? 0;
    $pay_week  = $payments_by_company[$cid]['week'] ?? 0;
    $pay_month = $payments_by_company[$cid]['month'] ?? 0;

    $exp_today = $expenses_by_company[$cid]['today'] ?? 0;
    $exp_week  = $expenses_by_company[$cid]['week'] ?? 0;
    $exp_month = $expenses_by_company[$cid]['month'] ?? 0;
    ...
    $branch_metrics[] = [
        ...
        'enrollments' => ['today' => $enroll_today, 'week' => $enroll_week,'month' => $enroll_month],
...
$att_from_date = $_POST['att_from_date'] ?? date('Y-m-d');
$att_to_date   = $_POST['att_to_date'] ?? date('Y-m-d');
...
if (!empty($att_from_date)) {
    $att_where .= " AND a.attendance_date >= :att_from_date";
    $att_params[':att_from_date'] = $att_from_date;
}
if (!empty($att_to_date)) {
    $att_where .= " AND a.attendance_date <= :att_to_date";
    $att_params[':att_to_date'] = $att_to_date;
}
    ...
    <div class="main-content">
            ...
            <div class="branch-card">
                ...
                <div class="metrics-container">
                    ...
                    <div class="metric-item green">
                        ...
                        <div class="metric-value"><?php echo $bm['enrollments']['month']; ?></div>
                        <div class="metric-subtext">Today: <?php echo $bm['enrollments']['today']; ?> | This Week: <?php echo $bm['enrollments']['week']; ?></div>
                    </div>
        ...
        <div class="premium-table-container">
            <form method="POST" action="#attendance-section" class="filter-panel">
                ...
                <div class="filter-group">
                    ...
                    <input type="date" name="att_from_date" class="filter-input" value="<?php echo htmlspecialchars($att_from_date); ?>">
                </div>

                <div class="filter-group">
                    <label>To Date</label>
                    <input type="date" name="att_to_date" class="filter-input" value="<?php echo htmlspecialchars($att_to_date); ?>">
                </div>
        ...
        <div class="premium-table-container">
            <form method="POST" action="#payroll-section" class="filter-panel">
                ...
                <input type="hidden" name="att_from_date" value="<?php echo htmlspecialchars($att_from_date); ?>">
                <input type="hidden" name="att_to_date" value="<?php echo htmlspecialchars($att_to_date); ?>">
                                ...
                                <tr class="accordion-content" id="pay-month-<?php echo $period_key; ?>" style="display: none; background: #f8fafc;">
                                    <td colspan="8" style="padding: 1.5rem;">
                                        <div style="background: white; border-radius: 0.75rem; border: 1px solid #e2e8f0; padding: 1.25rem; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02);">
                                            <?php 
                                            $exec_date = '';
                                            if (!empty($p_data['records']) && !empty($p_data['records'][0]['payment_date'])) {
                                                $exec_date = date('d-m-Y', strtotime($p_data['records'][0]['payment_date']));
                                            }
                                            ?>
                                            <h4 style="margin: 0 0 1rem 0; color: #1e293b; font-size: 1rem;"><i class="fas fa-file-invoice-dollar" style="color: #10b981; margin-right: 0.5rem;"></i> Detailed Payroll Summary for <?php echo htmlspecialchars($p_data['period']); ?> (<?php echo htmlspecialchars($p_data['branch_name']); ?> Branch)<?php echo !empty($exec_date) ? ' - Excuted on ' . $exec_date : ''; ?></h4>
                                            <table class="premium-table" style="width: 100%; margin: 0; box-shadow: none; border: 1px solid #f1f5f9;">
                                                <thead>
                                                    <tr style="background: #f8fafc;">
                                                        <!-- <th>Payroll Date</th> -->
                                                ...
                                                <tbody>
                                                    ...
                                                    ?>
                                                        <tr>
                                                            <?php /* <td><code style="color: #6d28d9; font-weight:600;"><?php echo date('d-m-Y', strtotime($rec['payment_date'])); ?></code></td>
                                                            <td><code style="color: #0284c7;"><?php echo htmlspecialchars($rec['staff_number']); ?></code></td> */ ?>
                                                            <td><strong><?php echo htmlspecialchars($rec['staff_name']); ?></strong></td>
                                                            <td><span class="badge-branch"><?php echo htmlspecialchars($rec['branch_name'] ?? 'Main'); ?></span></td>
