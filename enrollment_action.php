...
if ($action === 'search_students') {
    ...
    $query = "SELECT * FROM " . $GLOBALS['enrollment_table'] . " WHERE student_name LIKE :search AND deleted = 0 AND company_id = :comp_id ORDER BY id DESC";
    $params = [
        ':search' => '%' . $search . '%',
        ':comp_id' => $_SESSION['company_id']
    ];
    $records = $bf->getQueryRecords($query, $params);
    $results = [];
    $seen = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['student_name'])) {
    ...
    $num_installments = $bf->sanitize($_POST['num_installments'] ?? '');
    ...
    if ($fees_type === 'Installment') {
        $res = $valid->common_validation($num_installments, 'Number of Installments', '');
        if (!$res && (!ctype_digit($num_installments) || intval($num_installments) <= 0)) {
            $res = "Number of Installments must be a positive integer";
        }
        if ($res) $errors['num_installments'] = $res;
    } else {
    ...
    $res = $valid->valid_date($dob, 'Date Of Birth', 'text');
    if ($res) $errors['dob'] = $res;
    ...
    if (empty($errors)) {
        ...
        if (empty($view_enrollment_id)) {
            // Check for duplicate student-course enrollment
            $check_dup = $bf->getQueryRecords(
                "SELECT id FROM " . $GLOBALS['enrollment_table'] . " WHERE student_id = :student_id AND course_id = :course_id AND deleted = 0",
                [':student_id' => $student_id, ':course_id' => $course_id]
            );
            if (!empty($check_dup)) {
                echo json_encode([
                    'status' => 'error',
                    'errors' => ['student_name' => 'This student is already enrolled in this course.'],
            ...
            // Custom enrollment number: ENT003/26-27
            ...
            $max_query = $bf->getQueryRecords("SELECT MAX(id) as max_id FROM " . $GLOBALS['enrollment_table']);
            $next_num = (!empty($max_query[0]['max_id']) ? intval($max_query[0]['max_id']) : 0) + 1;
            $seq_str = str_pad($next_num, 3, '0', STR_PAD_LEFT);
            $data['enrollment_number'] = "ENT" . $seq_str . $year_suffix;
            
            // Set student username and password directly in the enrollment table
            ...
            // Fetch the generated enrollment_id
            $enrollment_record = $bf->getTableRecords($GLOBALS['enrollment_table'], 'id', $new_id);
            $target_enrollment_id = !empty($enrollment_record) ? $enrollment_record[0]['enrollment_id'] : '';

            // Regenerate Installment Schedule for Training
            if (!empty($target_enrollment_id)) {
                $stmt = $bf->con->prepare("DELETE FROM " . $GLOBALS['installments_table'] . " WHERE enrollment_id = :enroll_id AND course_type = 'training'");
                $stmt->execute([':enroll_id' => $target_enrollment_id]);
                
                if ($fees_type === 'Installment' && !empty($num_installments) && !empty($doj) && !empty($duration)) {
                    $duration_numeric = floatval($duration);
                    if ($duration_numeric > 0) {
                        $interval_months = $duration_numeric / $num_installments;
                        $installment_amount = round(floatval($fees_amount) / $num_installments, 2);
                        $cumulative_amount = 0;
                        
                        for ($i = 1; $i <= $num_installments; $i++) {
                            $months_to_add = round($interval_months * ($i - 1));
                            $due_date = date('Y-m-d', strtotime($doj . " + " . $months_to_add . " months"));
                            
                            if ($i == $num_installments) {
                                $amount = floatval($fees_amount) - $cumulative_amount;
                            } else {
                            ...
                            $bf->InsertSQL($GLOBALS['installments_table'], $inst_data, '', '', 'ADD INSTALLMENT SCHEDULE');
                        }
                    }
                }
            }
            ...
            if (!empty($from_enquiry)) {
                $enrollment_record = $bf->getTableRecords($GLOBALS['enrollment_table'], 'id', $new_id);
                $generated_enrollment_id = !empty($enrollment_record) ? $enrollment_record[0]['enrollment_id'] : '';
                $bf->UpdateSQL($GLOBALS['course_enquiry_table'], [
                    'converted_type' => 'enrollment',
                    'converted_id' => $generated_enrollment_id
                ], "enquiry_id = :enq_id", [':enq_id' => $from_enquiry]);
            }
            
            if (floatval($paid_amount) > 0) {
                // Fetch the generated enrollment_id from the inserted record
                $enrollment_record = $bf->getTableRecords($GLOBALS['enrollment_table'], 'id', $new_id);
                $generated_enrollment_id = !empty($enrollment_record) ? $enrollment_record[0]['enrollment_id'] : '';
                ...
                echo json_encode([
                    ...
                    'course_type' => 'training',
                    'student_id' => $student_id,
                    'message' => 'Enrollment added. Please complete the receipt.'
                ]);
            } else {
            ...
            $target_enrollment_id = $view_enrollment_id;
   ...
   $student_id = ""; $student_name = ""; $father_spouse_name = ""; $address = ""; $mobile_number = ""; $parent_contact_no = ""; $course_id = ""; $num_installments = ""; $duration = ""; $from_time = ""; $to_time = ""; $staff_id = ""; $fees_type = ""; $fees_amount = ""; $paid_amount = "0"; $balance_amount = ""; $dob = ""; $doj = ""; $blood_group = ""; $candidate_photo = ""; $lead_source = ""; $referred_staff_id = "";
    ...
    if(isset($_REQUEST['view_enrollment_id'])) {
        ...
        if(!empty($enrollment_list)) {
            foreach($enrollment_list as $data) {
                ...
                if(!empty($data['num_installments'])) {
        ...
        if (empty($view_enrollment_id)) {
            ...
            $prefix = "WG" . $month_char . date('y');
            
            $records = $bf->getQueryRecords("SELECT student_id FROM " . $GLOBALS['enrollment_table'] . " WHERE deleted = 0");
            $max_seq = 0;
            foreach ($records as $r) {
        ...
        <div class="module-section form-section">
            ...
            >
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-4">
                        ...
                        <select name="course_id"
                            ...
                            <?php
                                ...
                                $course_list = $bf->getQueryRecords("SELECT course_id, course_name FROM " . $GLOBALS['course_table'] . " WHERE deleted = 0");
                    ...
                    <div class="form-group col-4 <?php if ($fees_type !== 'Installment') { ?> d-none <?php } ?>" id="num_installments_group">
                        ...
                        <input
                            ...
                            class="form-input"
                            value="<?php echo htmlspecialchars($num_installments); ?>"
                            onkeypress="return allowNumbersOnly(event)"
                            oninput="restrictPasteNumbers(event)"
                        >

                        <span id="error-num_installments" class="error-msg"></span>
                    </div>
                    ...
                    <div class="form-group col-4">
                        ...
                        <div id="paid_amount_suggestion" style="color: #2563eb; font-size: 0.85rem; margin-top: 0.25rem; font-weight: 500;"></div>
                    </div>
                ...
                <script>
                    ...
                    /* Paid Amount Suggestion Logic */
                    function updatePaidAmountSuggestion() {

                        var selectedFeesType = document.querySelector('input[name="fees_type"]:checked');
                        var feesAmountInput = document.querySelector('input[name="fees_amount"]');
                        var numInstallmentsSelect = document.querySelector('input[name="num_installments"]');
                        var paidAmountInput = document.querySelector('input[name="paid_amount"]');
                        var suggestionDiv = document.getElementById('paid_amount_suggestion');
                        
                        if (!suggestionDiv) return;


                        
                        if (selectedFeesType && selectedFeesType.value === 'Installment') {
                            var feesAmount = parseFloat(feesAmountInput.value) || 0;
                            var numInstallments = parseInt(numInstallmentsSelect.value) || 0;


                            
                            if (feesAmount > 0 && numInstallments > 0) {
                                var suggested = (feesAmount / numInstallments).toFixed(2);
                                suggestionDiv.innerHTML = "💡 Suggested installment amount: ₹" + parseFloat(suggested).toLocaleString('en-IN', {minimumFractionDigits: 2});
                                
                                if (paidAmountInput.value === '' || paidAmountInput.dataset.autoFilled === 'true') {
                                    paidAmountInput.value = suggested;
                                    paidAmountInput.dataset.autoFilled = 'true';
                                    calculateBalance();
                                }
                            } else {
                    ...
                    document.querySelectorAll('input[name="fees_type"]').forEach(function(radio) {
                        ...
                        radio.addEventListener('change', function() {
                            ...
                            var feesAmount = parseFloat(document.querySelector('input[name="fees_amount"]').value) || 0;
                            var numInstallmentsGroup = document.getElementById('num_installments_group');
                            var suggestionDiv = document.getElementById('paid_amount_suggestion');
                            ...
                            if(this.value === 'Full Payment') {
                                ...
                                if (numInstallmentsGroup) {
                                    numInstallmentsGroup.classList.add('d-none');
                                ...
                                if (numInstallmentsGroup) {
                                    numInstallmentsGroup.classList.remove('d-none');
                                }
                                updatePaidAmountSuggestion();
                            }
                    ...
                    var instInput = document.querySelector('input[name="num_installments"]');
                    if (instInput) {
                    ...
                    // Trigger on load for pre-populated edit forms
    ...
    <?php } else {
        ...
        <div class="table-responsive">
            <table>
                ...
                $courses = $bf->getQueryRecords("
                    SELECT course_id, course_name
                    FROM {$GLOBALS['course_table']}
                    WHERE deleted = 0
                ");

                $payment_enrollment_ids = [];
                ...
                foreach ($payments as $row) {
                    $payment_enrollment_ids[$row['enrollment_id']] = true;
                }

                foreach ($courses as $course) {
                    $course_lookup[$course['course_id']] = $course['course_name'];
                }
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display:flex; gap:0.5rem;">
                                <?php if (checkPermission($_SESSION['company_id'], $_SESSION['role_id'], 'enrollment', PERMISSION_EDIT)): ?>
                                ...
                                <?php endif; ?>
                                <?php if (checkPermission($_SESSION['company_id'], $_SESSION['role_id'], 'enrollment', PERMISSION_DELETE)): 
                                    // $relations = [
                                    //     ['table' => $GLOBALS['payment_table'], 'column' => 'enrollment_id', 'value' => $u['enrollment_id'], 'label' => 'Receipt']
                                    // ];
                                    // $linked = $bf->isRecordLinked($relations);
                                    $linked = isset(
                                        $payment_enrollment_ids[$u['enrollment_id']]
                                    );
                                    if ($linked) { ?>
                                        <button class="btn-add" style="padding: 0.25rem 0.75rem; font-size: 0.8rem; background: #94a3b8; cursor: not-allowed;" title="Cannot delete: Linked with <?php echo $linked; ?> table" onclick="alert('Cannot delete: this enrollment is linked with <?php echo $linked; ?>. Please delete linked receipts first.')">Delete</button>
                                    <?php } else { ?>
...
if (isset($action) &&  ($action == 'delete')) {
    ...
    $enrollment_id = $bf->getTableColumnValue($GLOBALS['enrollment_table'], 'id', $id, 'enrollment_id');
    $relations = [
        ['table' => $GLOBALS['payment_table'], 'column' => 'enrollment_id', 'value' => $enrollment_id, 'label' => 'Receipt']
    ];
    $linked = $bf->isRecordLinked($relations);
    if ($linked) {
        echo "Error: Cannot delete enrollment because it is linked with $linked table.";
        exit;
    }
