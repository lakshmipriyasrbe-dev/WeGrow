...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['student_name'])) {
    ...
    if ($fees_type === 'Installment') {
        ...
        if (!$res && (!ctype_digit($num_installments) || intval($num_installments) <= 0)) {
            $errors['num_installments'] = 'Please enter a valid positive number of installments';
        }
    ...
    if (empty($errors)) {
        ...
        if (empty($view_enrollment_internship_id)) {
            ...
            // Generate Installment Schedule for Internship
            $enrollment_record = $bf->getTableRecords($GLOBALS['enrollment_internship_table'], 'id', $new_id);
            $target_enrollment_id = !empty($enrollment_record) ? $enrollment_record[0]['enrollment_internship_id'] : '';
            if (!empty($target_enrollment_id)) {
                $stmt = $bf->con->prepare("DELETE FROM " . $GLOBALS['installments_table'] . " WHERE enrollment_id = :enroll_id AND course_type = 'internship'");
                ...
                if ($fees_type === 'Installment' && !empty($num_installments) && !empty($doj) && !empty($duration)) {
                    ...
                    if ($duration_numeric > 0) {
                        $interval_days = $duration_numeric / $num_installments;
                        $installment_amount = round(floatval($fees_amount) / $num_installments, 2);
                        $cumulative_amount = 0;
                        
                        for ($i = 1; $i <= $num_installments; $i++) {
                            $days_to_add = round($interval_days * ($i - 1));
                            $due_date = date('Y-m-d', strtotime($doj . " + " . $days_to_add . " days"));
            ...
            // Generate/Update Installment Schedule for Internship
            $target_enrollment_id = $view_enrollment_internship_id;
            if (!empty($target_enrollment_id)) {
                $stmt = $bf->con->prepare("DELETE FROM " . $GLOBALS['installments_table'] . " WHERE enrollment_id = :enroll_id AND course_type = 'internship'");
                $stmt->execute([':enroll_id' => $target_enrollment_id]);
   ...
   $student_id = ""; $student_name = ""; $father_spouse_name = ""; $address = ""; $num_installments = ""; $mobile_number = ""; $parent_contact_no = ""; $course_id = ""; $duration = ""; $from_time = ""; $to_time = ""; $staff_id = ""; $fees_type = ""; $fees_amount = ""; $paid_amount = "0"; $balance_amount = ""; $dob = ""; $doj = ""; $blood_group = ""; $candidate_photo = ""; $lead_source = ""; $referred_staff_id = "";
                        ...
                        <td>
                            <div style="display:flex; gap:0.5rem;">
                                ...
                                <?php if (checkPermission($_SESSION['company_id'], $_SESSION['role_id'], 'enrollment_internship', PERMISSION_DELETE)): 
                                    $linked = isset(
                                        $payment_enrollment_ids[$u['enrollment_internship_id']]
                                    );
                                    if ($linked) { ?>
