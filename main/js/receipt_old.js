...
function updateModalTotal() {
    ...
    const fees = total;
    const cgst = Math.round((fees * 0.09 + Number.EPSILON) * 100) / 100;
    const sgst = Math.round((fees * 0.09 + Number.EPSILON) * 100) / 100;
    const totalWithTax = fees + cgst + sgst;
    ...
    if (modalFeesInput) modalFeesInput.value = fees.toFixed(2);
    if (modalCgstInput) modalCgstInput.value = cgst.toFixed(2);
    if (modalSgstInput) modalSgstInput.value = sgst.toFixed(2);
    if (totalInput) {
        if (modalFeesInput) {
            totalInput.value = totalWithTax.toFixed(2);
        } else {
