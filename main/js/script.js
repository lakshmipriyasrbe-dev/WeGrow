function formSubmit(form_name, from_page, to_page, page_name) {
    ...
    // Show processing alert above the form
    var alertDiv = form.parentElement.querySelector('.processing-alert');
    if (alertDiv) alertDiv.remove();
    alertDiv = document.createElement('div');
    alertDiv.className = 'alert alert-info d-flex align-items-center processing-alert';
    alertDiv.setAttribute('role', 'alert');
    alertDiv.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status"></span> Processing, please wait...';
    ...
    fetch(from_page, {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                // Switch alert to danger
                alertDiv.className = 'alert alert-danger d-flex align-items-center processing-alert';
                alertDiv.innerHTML = '⚠️ ' + (data.message || 'Please fix the errors below.');
        ...
        .catch(error => {
            ...
            alertDiv.className = 'alert alert-danger d-flex align-items-center processing-alert';
            alertDiv.innerHTML = '⚠️ An unexpected error occurred.';
            if (submitBtn) {
                submitBtn.disabled = false;
                submitBtn.innerText = originalBtnText;
            }
...
// Function to wrap tables in scrollable responsive divs
function wrapResponsiveTables() {
    $('table.responsive, table:not(.permission-table):not(.payment-table)').each(function () {
        var $table = $(this);
        if (!$table.parent().hasClass('table-responsive') &&
            !$table.parent().hasClass('table-responsive-wrapper') &&
            !$table.parent().hasClass('schedule-table-wrapper') &&
            !$table.parent().hasClass('premium-table-container')) {
...
$(document).ready(function () {
    ...
    // MutationObserver to automatically handle AJAX loaded inputs and tables
    const observer = new MutationObserver(function (mutations) {
        let hasPasswordOrTable = false;
                ...
                mutation.addedNodes.forEach(function (node) {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        if (node.matches('input[type="password"]') || node.querySelector('input[type="password"]') ||
                            ...
                            hasPasswordOrTable = true;
                        }
                    }
                });
            }
        });
        if (hasPasswordOrTable) {
            initPasswordFields();
            wrapResponsiveTables();
        }
    });
