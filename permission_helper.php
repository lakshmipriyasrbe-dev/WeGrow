...
function checkPermission($company_id, $role_id, $access_page, $access_action) {
    ...
    static $is_management_cache = [];
    $is_management = false;
    if (isset($is_management_cache[$role_id])) {
        $is_management = $is_management_cache[$role_id];
    } else {
        try {
            ...
            $is_management_cache[$role_id] = $is_management;
        } catch (Exception $e) {
            // Fallback
        }
    }
    ...
    $cache_key = "{$company_id}_{$role_id}_{$access_page}";
    ...
    if (!isset($permissions_cache[$cache_key])) {
        try {
            ...
            $permissions_cache[$cache_key] = ($row && !empty($row['permission_action'])) ? explode('$$', $row['permission_action']) : [];
        } catch (PDOException $e) {
            $permissions_cache[$cache_key] = [];
        }
    }

    return in_array($access_action, $permissions_cache[$cache_key]);
}
?>
