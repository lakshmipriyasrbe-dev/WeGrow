...
if ($action == 'get_payment_modal') {
    ...
    <script>
        var modalPaymentModeOptions = `<?php echo addslashes($payment_mode_options); ?>`;
        var modalStudentLists = <?php echo $student_lists_json; ?>;
        var modalBankMap = <?php echo $bank_map_json; ?>;
        var modalPaymentRows = <?php echo $payment_rows_json; ?>;
        var modalSelectedCourseType = '<?php echo $modal_course_type; ?>';
        var modalSelectedStudentId = '<?php echo $modal_student_id; ?>';
        var enrollmentPaidAmount = parseFloat('<?php echo $enrollment_paid_amount; ?>') || 0;
...
if (isset($_REQUEST['view_receipt_id'])) {
    ...
    <script>
        var paymentModeOptions = `<?php echo addslashes($payment_mode_options); ?>`;
        var studentLists = <?php echo $student_lists_json; ?>;
        var bankMap = <?php echo $bank_map_json; ?>;
        var paymentRows = <?php echo json_encode($payment_rows); ?>;
        var selectedCourseType = '<?php echo $course_type; ?>';
        var selectedStudentId = '<?php echo $student_id; ?>';
        ...
        setTimeout(function() {
            if($.fn.select2) {
                ...
                // Keep selectedStudentId in sync when changed via Select2
                                ...
                                <?php if (checkPermission($_SESSION['company_id'], $_SESSION['role_id'], 'receipt', PERMISSION_VIEW)): ?>
                                    ...
                                    if (!empty($student_record)) {
                                        ...
                                        /* $wa_text = "Dear *" . ($student_record['student_name'] ?? 'Student') . "*, your payment has been received successfully! \n\n"
                                                 ...
                                                 . ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . "/admin/reports/rpt_receipt_a5.php?receipt_id=" . $receipt['id'] . "\n\n"
                                                 . "Thank you! 🙏"; */ 
                                        ...
                                        $wa_text = "Dear *" . ($student_record['student_name'] ?? 'Student') . "*, your payment has been received successfully! \n\n"
                                                 ...
                                                 . "*   *Total Paid:* Rs. " . number_format((float)($receipt['total_amount'] ?? 0), 2) . "\n\n"
                                                 . "📄 *View/Download PDF Receipt:*\n"
                                                 . ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . "/admin/reports/rpt_receipt_a5.php?receipt_id=" . $receipt['id'] . "&hash=" . $receipt_hash . "\n\n"
                                                 . "Thank you! 🙏";
