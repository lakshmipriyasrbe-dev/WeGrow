...
if ($action == 'list') {
    ...
    $query = "FROM " . $GLOBALS['report_table'] . " r 
              LEFT JOIN " . $GLOBALS['staff_table'] . " u ON r.user_id = u.staff_id 
              LEFT JOIN " . $GLOBALS['user_table'] . " usr ON r.user_id = usr.user_id
              WHERE r.deleted = :deleted AND r.company_id = :comp_id";
    $params = [':deleted' => 0, ':comp_id' => $_SESSION['company_id']];
    ...
    if (!empty($search)) {
        $query .= " AND (u.staff_name LIKE :search OR usr.name LIKE :search OR r.activity_details LIKE :search)";
        $params[':search'] = "%$search%";
    }
    ...
    $data_query = "SELECT r.*, COALESCE(u.staff_name, usr.name) as user_name, COALESCE(u.role, usr.role) as user_role " . $query . " ORDER BY r.report_date DESC LIMIT $start, $limit";
    $reports = $bf->getQueryRecords($data_query, $params);
