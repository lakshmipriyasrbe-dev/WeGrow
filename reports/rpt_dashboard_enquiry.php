...
if (empty($records)) {
    ...
    foreach ($records as $r) {
        ...
        $student_id_dec = (!empty($r['student_id'])) ? $bf->encode_decode('decrypt', $r['student_id']) : '';
        
        if (isset($r['source_type']) && $r['source_type'] === 'Enquiry') {
            $enroll_id_dec = $r['enroll_id'] ?? '';
        } else {
            $enroll_id_dec = (!empty($r['enroll_id'])) ? $bf->encode_decode('decrypt', $r['enroll_id']) : ($r['enroll_id'] ?? '');
            if ($enroll_id_dec === false || $enroll_id_dec === '') {
                $enroll_id_dec = $r['enroll_id'] ?? '';
            }
        }
