...
// Check authorization:
// 1. Logged in as admin, director, or management
// 2. Or a guest with a valid secure hash matching the receipt ID
$is_authorized = false;
if (isset($user_role) && ($user_role === 'admin' || $user_role === 'director' || $is_management || $user_role === 'manager')) {
    $is_authorized = true;
} else {
    $hash = $_REQUEST['hash'] ?? '';
    $expected_hash = md5($receipt_id . ($GLOBALS['salt'] ?? 'TrainingCenter_2026_Secure_Salt_!@#'));
    if (!empty($hash) && $hash === $expected_hash) {
        $is_authorized = true;
    }
}
...
if (!empty($payment_mode_ids)) {
    foreach ($payment_mode_ids as $pm_id) {
        // $pm_record = $bf->getTableRecords($GLOBALS['payment_mode_table'], 'payment_mode_id', $pm_id);
         $pm_record = $bf->getQueryRecords("SELECT payment_mode_name FROM " . $GLOBALS['payment_mode_table'] . " WHERE payment_mode_id = :pm_id", [':pm_id' => $pm_id]);
        if (!empty($pm_record)) {
...
if (!empty($bank_ids)) {
    foreach ($bank_ids as $bank_id) {
        if (!empty($bank_id)) {
            ...
            $bank_record = $bf->getQueryRecords("SELECT bank_name FROM " . $GLOBALS['bank_table'] . " WHERE bank_id = :bank_id", [':bank_id' => $bank_id]);
...
$course_data = $bf->getQueryRecords("SELECT course_name FROM " . $GLOBALS['course_table'] . " WHERE course_id = :course_id", [':course_id' => $student['course_id'] ?? '']);
// $course_name = !empty($course_data) ? substr($course_data[0]['course_name'], 0, 22) : '';
$course_name = !empty($course_data) ? $course_data[0]['course_name'] : '';
$pdf->Cell(60, 4.5, $course_name, 0, 1);
...
if ($is_receipt_gst_applicable) {
    ...
    $pdf->Cell($col_widths[0] + $col_widths[1] + $col_widths[2], 5, 'FEES AMOUNT', 1, 0, 'R', true);
    $pdf->Cell($col_widths[3], 5, 'Rs. ' . number_format($base_fees, 2), 1, 1, 'R', true);

    $pdf->Cell($col_widths[0] + $col_widths[1] + $col_widths[2], 5, 'CGST (9%)', 1, 0, 'R', true);
    $pdf->Cell($col_widths[3], 5, 'Rs. ' . number_format($cgst_val, 2), 1, 1, 'R', true);

    $pdf->Cell($col_widths[0] + $col_widths[1] + $col_widths[2], 5, 'SGST (9%)', 1, 0, 'R', true);
    $pdf->Cell($col_widths[3], 5, 'Rs. ' . number_format($sgst_val, 2), 1, 1, 'R', true);

    $pdf->SetFillColor(240, 249, 255);
    $pdf->SetTextColor(14, 165, 233);
    $pdf->Cell($col_widths[0] + $col_widths[1] + $col_widths[2], 6, 'TOTAL AMOUNT', 1, 0, 'R', true);
    $pdf->Cell($col_widths[3], 6, 'Rs. ' . number_format($grand_total, 2), 1, 1, 'R', true);
} else {
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->SetDrawColor(14, 165, 233);
    $pdf->SetFillColor(240, 249, 255);
...
// Computer generated message at bottom of A5 Page
$pdf->SetAutoPageBreak(false);
$pdf->SetY(-15);
$pdf->SetFont('Arial', 'I', 8);
$pdf->SetTextColor(100, 116, 139);
$pdf->Cell(0, 4, 'This is a computer generated bill, signature not mandatory.', 0, 1, 'C');

$pdf->SetY(-5);
$pdf->SetFont('Arial', 'I', 7);
$pdf->SetTextColor(148, 163, 184);
// $pdf->Cell(0, 3, 'Generated on ' . date('d-m-Y H:i:s'), 0, 1, 'C');
$pdf->Cell(0, 3, 'Amount once paid will not be refunded.', 0, 1, 'C');
